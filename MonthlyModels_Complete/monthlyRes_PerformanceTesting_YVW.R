# Monthly Res Model Performance Testing Across Training Periods
...
# Read in monthly residential data
...
for (i in 1:length(initialDates$Month)) {
  pred6[,((2*i)-1)] <- monthlyData %>% filter(Month >= as.yearmon(initialDates$Date[i]))  %>% filter(Month <= as.yearmon(initialDates$Date[i] + (predictPeriod-1)/12)) %>% select(Res)
  tempForecastSet <- monthlyResForecastYVW(monthlyData, initialPredictMonth = initialDates$Month[i], initialPredictYear = initialDates$Year[i], predictPeriod = predictPeriod, trainPeriod = 6)
...
for (i in 1:length(initialDates$Month)) {
  pred12[,((2*i)-1)] <- monthlyData %>% filter(Month >= as.yearmon(initialDates$Date[i]))  %>% filter(Month <= as.yearmon(initialDates$Date[i] + (predictPeriod-1)/12)) %>% select(Res)
  tempForecastSet <- monthlyResForecastYVW(monthlyData, initialPredictMonth = initialDates$Month[i], initialPredictYear = initialDates$Year[i], predictPeriod = predictPeriod, trainPeriod = 12)
...
for (i in 2:length(initialDates$Month)) {
  pred18[,((2*i)-1)] <- monthlyData %>% filter(Month >= as.yearmon(initialDates$Date[i]))  %>% filter(Month <= as.yearmon(initialDates$Date[i] + (predictPeriod-1)/12)) %>% select(Res)
  tempForecastSet <- monthlyResForecastYVW(monthlyData, initialPredictMonth = initialDates$Month[i], initialPredictYear = initialDates$Year[i], predictPeriod = predictPeriod, trainPeriod = 18)
...
for (i in 2:length(initialDates$Month)) {
  pred24[,((2*i)-1)] <- monthlyData %>% filter(Month >= as.yearmon(initialDates$Date[i]))  %>% filter(Month <= as.yearmon(initialDates$Date[i] + (predictPeriod-1)/12)) %>% select(Res)
  tempForecastSet <- monthlyResForecastYVW(monthlyData, initialPredictMonth = initialDates$Month[i], initialPredictYear = initialDates$Year[i], predictPeriod = predictPeriod, trainPeriod = 24)
...
for (i in 3:length(initialDates$Month)) {
  pred36[,((2*i)-1)] <- monthlyData %>% filter(Month >= as.yearmon(initialDates$Date[i]))  %>% filter(Month <= as.yearmon(initialDates$Date[i] + (predictPeriod-1)/12)) %>% select(Res)
  tempForecastSet <- monthlyResForecastYVW(monthlyData, initialPredictMonth = initialDates$Month[i], initialPredictYear = initialDates$Year[i], predictPeriod = predictPeriod, trainPeriod = 36)
...
for (i in 4:length(initialDates$Month)) {
  pred48[,((2*i)-1)] <- monthlyData %>% filter(Month >= as.yearmon(initialDates$Date[i]))  %>% filter(Month <= as.yearmon(initialDates$Date[i] + (predictPeriod-1)/12)) %>% select(Res)
  tempForecastSet <- monthlyResForecastYVW(monthlyData, initialPredictMonth = initialDates$Month[i], initialPredictYear = initialDates$Year[i], predictPeriod = predictPeriod, trainPeriod = 48)
...
png('monthlyResPerformanceMAPE_YVW.png', height = 50*nrow(mapePerformance), width=200*ncol(mapePerformance))
...
png('monthlyResPerformanceRMSE_YVW.png', height = 50*nrow(RMSEPerformance), width=200*ncol(RMSEPerformance))
